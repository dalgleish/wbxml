...
function isEmptyNode(node, type) {
  return (type === 'undefined' || node === null);
}
...
var _inspect = function (obj) {
  ...
  function parseElement(name, currentPage) {
    ...
    if(element.length > 1) {
      page = nameSpaces.indexOf(element[0]) || currentPage;
      element = element[1];
    } else {
      element = name;
      page = currentPage;
  ...
  traverse(obj).forEach(function() {
    ...
    if (!isEmptyNode(node, type) && !util.isArray(node) && type !== 'object') {
